...
If you want to better understand how to implement your own IncrementalCache, the easiest way would be to take a look at the existing included IncrementalCache [here](https://github.com/opennextjs/opennextjs-aws/tree/main/packages/open-next/src/overrides/incrementalCache).
...
All the cache file are present in the `.open-next/cache` folder. The one under `BUILD_ID` are for the ISR/SSG cache and the one under `__fetch/BUILD_ID` are for the fetch cache.