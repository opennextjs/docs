...
It works with function splitting in OpenNext aswell. If your key corresponds to a specific route (i.e `api/test`), it will only get copied into that functions bundle. However, using `*` as key will copy it into every function bundle. Here is an example with function splitting:
