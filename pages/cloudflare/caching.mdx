...
<Callout>
  ...
  You can also skip this step if you don't want to use `revalidateTag` or `revalidatePath` in your app.
</Callout>

There is 3 different options to choose from for the tag cache: 'd1NextTagCache', `ShardedTagCache` and `d1TagCache`.
For a small to medium size project, `d1NextModeTagCache` is recommended, for bigger projects `ShardedTagCache` is recommended.
