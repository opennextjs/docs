...
- use regional cache by wrapping the handler into `withRegionalCache(...)`
  ...
  - you should not need to explictely set `shouldLazilyUpdateOnCacheHit` nor `bypassTagCacheOnCacheHit` as they are set to the most performant mode by default
